...
// ===== MQTT SERVER UBUNTU =====
const char* MQTT_HOST = "10.62.149.182"; //sửa ip server
...
bool initCamera() {
  ...
  // Nếu chụp biển số mờ, thử SVGA hoặc XGA nếu nguồn ổn
...
void flushCameraFrames(int count = 3, int delayMs = 120) {
...
void publishResult(const String &status, const String &uid, const String &token) {
  ...
  msg += "\"uid\":\"" + uid + "\",";
  msg += "\"token\":\"" + token + "\",";
  msg += "\"photo_url\":\"http://" + WiFi.localIP().toString() + "/photo?t=" + token + "\"";
  msg += "}";
...
bool captureAndStore() {
  ...
  // Bỏ 2 frame đầu để tránh ảnh rác / ảnh cũ
  flushCameraFrames(2, 100);
...
void handleRoot() {
  ...
  html += "<h2>ESP32-CAM OUT</h2>";
  html += "<p>IP: " + WiFi.localIP().toString() + "</p>";
  html += "<p>UID: " + lastUID + "</p>";
  html += "<p>Status: " + lastStatus + "</p>";
  html += "<p>Token: " + lastToken + "</p>";
  html += "<p><a href='/photo?t=" + String(millis()) + "'>Xem anh moi nhat</a></p>";
  html += "</body></html>";
...
void setup() {
  ...
  if (!initCamera()) {
	...
	Serial.println("Warm up camera...");
	flushCameraFrames(5, 150);
